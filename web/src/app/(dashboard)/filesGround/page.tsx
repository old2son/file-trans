		...
		openModal({
			content: (
				<div className="p-4">
					...
					<button
						onClick={closeModal}
						className="px-4 py-2 w-full bg-blue-500 text-white rounded hover:bg-blue-600 cursor-pointer"
					>
						关闭
					</button>
				</div>
			...
			<ul className="grid grid-cols-1 gap-5 px-5 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3">
				{items.map((item, index) => (
					...
					>
						...
						{item.text && (
							<p className="py-2.5 text-xl sm:text-sm text-center text-[#33333b]">{item.text}</p>
						)}
					</li>
				))}
			</ul>
